"""Bench Gemma 4 E2B 1-stage v2_beam on alpha GPU."""
import os, time, statistics, numpy as np
import openvino as ov

SHARD_DIR = r"C:\cascadia\shards_e2b_1stage_v2_beam"
DEVICE = "GPU"
...
def main():
    ...
    core = ov.Core()
    print("compiling stage_0 (1-stage v2_beam)...", flush=True)
    ...
    def decode_one(ids, max_tokens):
        ...
        if pos_name: feed[pos_name] = pos_np
        if has_beam: feed["beam_idx"] = beam
        res = r.infer(feed)
        ...
        for step in range(max_tokens - 1):
            ...
            if pos_name: feed[pos_name] = pos_one
            if has_beam: feed["beam_idx"] = beam
            res = r.infer(feed)

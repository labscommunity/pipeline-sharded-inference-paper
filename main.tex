...
\textbf{WAN characterization uses sleep-simulated latency.} The sweep in Table~\ref{tab:wan} injects \texttt{time.sleep()} on the worker's receive and send boundaries, which adds deterministic one-way delay per target forward but lacks real TCP-level effects (slow-start, jitter, retransmit-under-loss). The sleep-sim is a lower bound on real-WAN speedup (real TCP amortizes more in-flight messages per RTT); confirming this on a real-network testbed is future work.

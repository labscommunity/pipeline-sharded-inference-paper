...
Per-stage splitting costs $\sim$11--13\% on a single machine relative to $A_{\text{spec}}=24.28$; the per-stage GEMMs are smaller, reducing XVE occupancy (measured $60.1\%$ on mono vs.\ $43.6\%$ on 3-stage via VTune \texttt{gpu-hotspots}, with identical top-kernel counts), and the Python-level \texttt{hidden\_states.astype(np.float32)} cast between stages is not free. This gap is structural: smaller per-stage problems cannot saturate the GPU the way a single large problem does. We return to how to recover this cost (and then some) via multi-user micro-batching in~\S\ref{sec:microbatch} and via speculative decoding in~\S\ref{sec:spec}.

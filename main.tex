...
Modern AI PCs ship with integrated GPUs and 16--32~GB of unified memory, yet spend most of their time idle. A single Intel AI PC runs Llama~3.1~8B at 22~tokens/second. A fleet of them, coordinated over a standard office network, could serve LLM inference without cloud dependency---at zero marginal cost, with full data locality.
...
\begin{table}[ht]
\centering
\caption{Mapping the four contributions of \S\ref{sec:intro} to the three operating scenarios.}
...
3. Micro-batching via independent \texttt{InferRequest}s (\S\ref{sec:microbatch}) & --- & --- & \checkmark \\
4. Multiplicative composition $+$ WAN behavior (\S\ref{sec:distributed}) & \checkmark & \checkmark & \checkmark \\
\bottomrule
\end{tabular}
\end{table}

Contribution~1 (the export pipeline and the \texttt{beam\_idx} fusion unlock) underlies all three scenarios: it produces the shards everything else runs on. Contribution~2 (mask-based KV rewind) is what makes speculative decoding affordable on stateful OpenVINO, so it enters in scenarios~(b) and~(c). Contribution~3 enters in scenario~(c). Contribution~4 spans all three: it is the empirical demonstration that (a)$\rightarrow$(b)$\rightarrow$(c) compose multiplicatively, plus the behavior of each scenario under WAN latency. \S\ref{sec:scenario_compare} compares the three scenarios head-to-head on the same topologies and discusses which to deploy when.
...
The system assumes a trusted, reliable network. There is no fault tolerance, authentication, or encryption. WiFi latency varies by $\pm$2~ms (averaging out over many tokens). All nodes must run identical software stacks (same OpenVINO, same Python). The scope is integrated GPUs: CPU-hosted stages work (Table~\ref{tab:gemma_dist}) but same-SoC CPU stages contend with the GPU for memory bandwidth, and the NPU is out of scope because the OpenVINO NPU plugin compiles only static-shape graphs---it cannot run the dynamic stateful \texttt{ReadValue}/\texttt{Assign} KV-cache shards this system is built on---and a same-SoC NPU stage shares LPDDR bandwidth with the iGPU, so it could not raise memory-bandwidth-bound decode throughput even if it could.

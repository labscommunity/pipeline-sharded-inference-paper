...
\begin{enumerate}[leftmargin=*]
  ...
  \item \textbf{Multiplicative composition into a distributed stack that beats monolithic single-user throughput and degrades gracefully under WAN latency.} The three techniques compose empirically: the two-node fleet reaches $41.25$~tok/s at LAN ($1.89\times$ monolithic single-user), remains interactive at $100$~ms/hop simulated WAN ($4\times$ over naïve distributed decode), and scales to $54.92$~tok/s at $3$-stream on a three-node testbed~(\S\ref{sec:distributed}). A 4-stage Llama~3.1~70B INT4 deployment on Intel Tiber Cloud demonstrates the pipeline extends to model sizes that do not fit on any single fleet member~(\S\ref{sec:llama70b}). Top-1 logits compression recovers interactive throughput on relay-mediated WAN paths~(\S\ref{sec:topk_compress}).
\end{enumerate}
...
The 3-stage 2-stream stack hits $42.91$~tok/s at $K\!=\!5$, $4\%$ above the 2-stage $K\!=\!3$ headline ($41.25$); the third stage adds compute parallelism for mbatch (each stream keeps three iGPUs busy) without giving up bit-exactness. Going from 2-stream to 3-stream at $K\!=\!5$ adds $+12.0$~tok/s aggregate (from $42.91$ to $54.92$) at the cost of $\sim 3$~tok/s per-user latency, hitting the $\min(N_\text{users}, N_\text{stages})$ pipeline-fill ceiling exactly. Per-stream \texttt{compile\_model} (the documented OV \texttt{reset\_state} workaround, \S\ref{sec:ov_bug}) consumes $\sim 6$~GB iGPU per stream on the 16~GB Lunar Lake shared budget. Per-stream throughput at $18.4$~tok/s remains $\sim 3.7\times$ the $5$~tok/s interactive floor.

...
\begin{abstract}
We distribute large language model inference across fleets of commodity Intel AI PCs using pre-compiled pipeline shards composed with mask-based speculative decoding. Per-stage INT4 OpenVINO IR subgraphs are exported via \texttt{torch.jit.trace} with precomputed rotary embeddings and a post-export \texttt{beam\_idx} Gather injection that unlocks the OpenVINO GPU plugin's \texttt{IndirectKVCache} fusion, producing shards at parity with monolithic inference. A mask-based KV-cache rewind makes speculative decoding practical on stateful OpenVINO models without paged attention---masking rejected-draft cache positions via the existing \texttt{attention\_mask} input is bit-exact with physical trim at essentially zero cost. Combined with multi-user micro-batching via independent stateful \texttt{InferRequest}s, a two-node Llama~3.1~8B INT4 pipeline serves two concurrent users at $1.79\times$ the throughput of monolithic single-user inference on the same hardware; the advantage grows under simulated WAN latency. The pipeline scales to 70B: a 4-node deployment across Lunar Lake AI PCs on Intel Tiber Cloud reaches interactive throughput on a model that does not fit on any single fleet member, validated bit-exact against the same-topology target-only baseline. Code, raw benchmark logs, and reproduction scripts are released as a self-contained reproduction package alongside this paper at \texttt{github.com/labscommunity/pipeline-sharded-inference-paper} (under the top-level \texttt{reproduction/} directory; OS to be opened on publication).
\end{abstract}

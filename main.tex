...
+ Top-1 logits compression & \textbf{22.12} & \textbf{11.07} & 0.54$\times$ (1.9$\times$ slower) \\
\bottomrule
\end{tabular}
\end{table}
...
The 2-stream micro-batch row uses a Gemma-specific multi-stream coordinator and worker (per-stream \texttt{compile\_model} on both sides as the documented workaround for the \texttt{reset\_state()} multi-\texttt{InferRequest} bug in \S\ref{sec:ov_bug}). Aggregate throughput is $\mathbf{16.30}$~tok/s on v2 ($1.57\times$ over single-stream), $21\%$ above the paper's $13.51$. The mbatch \emph{ratio} ($1.57\times$) is below the paper's $1.66\times$ because the v2 single-stream baseline is itself higher --- there is less stage-idle time to fill on a $10.40$ baseline than on an $8.12$ one. Output between the two concurrent streams is byte-identical (same prompt, deterministic generation), confirming that the cross-stream KV isolation is correct.

...
Serving systems built on paged attention get prefix caching---reusing the KV cache of an already-processed prompt prefix instead of re-prefilling it---from their block-level cache managers~\cite{kwon2023vllm}. A stateful OpenVINO shard offers no such API: its KV cache is opaque per-\texttt{InferRequest} device state. Sharding nonetheless does not preclude prefix caching, and our implementation now supports a capture-and-resume form of it built from the same \texttt{query\_state()} machinery whose per-call cost we measured in \S\ref{sec:spec}---affordable here because capture runs once per generation rather than once per decode step, so the ${\sim}48$~ms state round-trip of Table~\ref{tab:trim_cost} that was fatal for per-step rewind is noise against a full prefill.

At the end of each generation, every stage serializes its own layer range's KV state into a self-describing blob (one \texttt{query\_state()} pass over the per-layer state tensors), keyed by the full token sequence it represents and held in a small LRU. When a new prompt is admitted, the coordinator looks for the longest cached sequence that is a strict prefix of the new prompt---the common case being a multi-turn conversation, where the client resends the history and the previous turn's full sequence is exactly such a prefix. On a hit, a restore message propagates down the stage chain with all-or-nothing semantics: every stage restores its captured state, or all stages abort back to a cold prefill. Only the uncached suffix is then prefilled, in a single batched forward, with positions derived from the restored cache depth rather than the matched token count. The capture and restore messages ride the same persistent inter-stage connections that carry activations (\S\ref{sec:relay}). The warm path is byte-identical to a cold prefill of the full prompt---certified on the single-stage configuration; the multi-stage protocol, each stage capturing and restoring only its own layers, runs end-to-end on fleet hardware but has not yet received the same byte-exact certification.

Two design points connect back to the rest of the paper. First, speculative decoding under mask-based rewind (\S\ref{sec:spec}) deliberately leaves rejected-draft positions physically in the cache, masked out. Captured state is therefore compacted at serialization time---only mask-valid positions survive into the blob---so a spec-decoded session is warm-resumable, with the draft model (which trails the target by one token) realigned before the shared suffix feed. Second, because the serialized state is opaque to everything but the producing shard, reuse is whole-sequence: a captured sequence accelerates any later prompt it prefixes, which covers session resume but not block-level sharing of partial prefixes between unrelated requests the way paged-attention systems allow. One practical caveat: chat templates that rewrite earlier turns (e.g., models that re-render thinking blocks) destroy prefix stability, and such prompts miss safely to a cold prefill---as does every failure path (missing capture on any stage, model-fingerprint mismatch, restore timeout). Quantifying the time-to-first-token savings on long conversations is future work; the correctness half (bit-exact warm vs.\ cold) is what we certify today.

\subsection{NPU Stages: Static, Stateless Shards}
\label{sec:npu}

Every benchmark in this paper runs its pipeline stages on integrated GPUs, and an earlier version of this system treated the NPU in every Core Ultra SoC as out of scope: the OpenVINO NPU compiler rejects both dynamic shapes and \texttt{ReadValue}/\texttt{Assign} state variables, which rules out the stateful dynamic-shape shards of \S\ref{sec:export} entirely. Our implementation has since gained a parallel export path that meets the NPU on its own terms; we document it here because the changes it required say something general about what portability across AI PC accelerators costs.

An NPU shard must be static-shape \emph{and} stateless. The NPU export path pins every input dimension (batch~1, decode sequence length~1, a context window fixed at export time) and skips the make-stateful transformation, so the KV cache becomes explicit graph ports: each shard is a pure function $(\text{input},\ \text{attention mask},\ \text{position ids},\ \text{past KV}) \rightarrow (\text{output},\ \text{present KV})$. One export subtlety: the static-shape assertion must re-run \emph{after} INT4 weight compression, because a decompression subgraph can reintroduce a dynamic dimension that the CPU and GPU plugins silently tolerate and that surfaces only as an NPU load failure.

What the compiled graph no longer does, the host does. The runtime keeps a per-stage bounded KV ring holding the most recent tokens' K/V and rewrites the fixed-length attention mask each step; with the sequence dimension pinned to~1 there is no chunked prefill, so the prompt feeds token-by-token through the same path. The sharding-specific piece is position lockstep. Stateful shards each track absolute position locally and resynchronize when a multi-token prefill activation arrives; a static shard never sees one, so stage~0 transmits the absolute position as a small framed tensor ahead of each hidden-state activation, and every downstream stage derives its ring reset and visible-past count from it, keeping all stages' rings in lockstep.

This path is validated for correctness, not throughput: a 2-stage Qwen2.5-1.5B INT4 pipeline with both stages compiled to and executing on a Lunar Lake NPU produces correct output on all test prompts, and heterogeneous pipelines that assign stages across iGPU, NPU, and CPU run end-to-end under an ILP-based placement solver fed by per-(stage, device) latency, memory, and op-support profiles. The measurements that solver consumes are also why no NPU numbers appear in our results tables: per-stage decode latency on the same SoC ranks GPU $<$ CPU $<$ NPU (an NPU stage costs roughly $4\times$ the iGPU per token on a 9B-class model), and because the NPU shares LPDDR bandwidth with the iGPU, offloading a stage to it does not raise memory-bandwidth-bound decode throughput---the placement solver never selects the NPU on throughput grounds. The value of the NPU path is capability rather than speed: sharding brings per-stage footprint within NPU limits (a monolithic 8B-class model fails to fit on the NPU, while per-stage shards of a 9B model compile and run there), and an NPU stage frees the iGPU for other work at lower power. The remaining constraints are real: context length fixed at export, batch~1, FP16 KV, minutes-long static-graph compiles, and per-model NPU compiler op support (Llama and Qwen2.5-family shards compile; Qwen3's exported graph currently does not).

\subsection{Limitations}

The system assumes a trusted, reliable network. There is no fault tolerance, authentication, or encryption. WiFi latency varies by $\pm$2~ms (averaging out over many tokens). All nodes must run identical software stacks (same OpenVINO, same Python). Every benchmark reported is scoped to integrated GPUs: CPU-hosted stages work (Table~\ref{tab:gemma_dist}) but same-SoC CPU stages contend with the GPU for memory bandwidth, and while NPU stages are now supported through the separate static, stateless export path of \S\ref{sec:npu} (the NPU plugin cannot compile the dynamic stateful \texttt{ReadValue}/\texttt{Assign} shards the rest of the paper is built on), that path is validated for correctness only---a same-SoC NPU stage shares LPDDR bandwidth with the iGPU, so it does not raise memory-bandwidth-bound decode throughput.
